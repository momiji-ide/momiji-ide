...
- **Full installer:** `npm run dist` (electron-builder, outputs to `dist/`)
- **CI:** GitHub Actions builds on push to `main` (see `.github/workflows/`)
- **Auto-update:** `electron-updater` checks GitHub Releases

Version bump procedure: edit `package.json` `version` → update `docs/index.html` (landing page badges + download links) → commit → push → CI builds and uploads to Releases.
...
_Last updated: handed over by Claude Sonnet 4.6 mid-session (v1.3.1 era). Subsequent agents should keep this file in sync as conventions evolve and as roadmap items ship. Move completed §11 items into §6 (anti-patterns) or §5 (how-to) so future agents learn from them._
