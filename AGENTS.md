...
2026-05-25 | Claude Opus 4.8 | Robot sim fixes — line-follower geometry (lateral sensors fwd=8/side=6, start ON oval at angle=0), 3D uses fixed CW×CH + CSS stretch (no setTimeout/DOM-dimension reads), BTN A/B UX hints | RobotSimulator.tsx
2026-05-25 | Claude Opus 4.8 | Code → Flowchart feature — codeToFlowchart.ts (JS brace parser + Python indent parser → nodes/edges with auto-layout), CodeFlowchart.tsx (React Flow w/ terminator/process/decision-diamond/io-parallelogram shapes), 📊 Flowchart toggle in EditorToolbar → split view in CodeEditor, examples/flowchart/ samples | codeToFlowchart.ts, CodeFlowchart.tsx, CodeEditor.tsx, EditorToolbar.tsx
2026-05-25 | Claude Opus 4.8 | Flowchart goes global — brace parser now handles all C-family langs (Java/C/C++/C#/Go/Rust/PHP/Swift/Kotlin/Dart) not just JS. readHeader is newline+semicolon aware (Go no-paren for-loops, Swift/Kotlin no-semicolon), isFlowchartSupported() helper gates the toolbar button. Verified Java/Go/Rust/C++ output | codeToFlowchart.ts, CodeFlowchart.tsx, CodeEditor.tsx, EditorToolbar.tsx
2026-05-25 | Claude Opus 4.8 | Kitsune Creative — prompt → shader in ShaderPlayground. Natural-language prompt bar calls callKitsune() with a strict WebGL1 GLSL system prompt, strips fences, compiles live. Iterate chips ("make it faster", "add glow") feed the current shader back for compounding edits. First slice of the AI-native creative loop | ShaderPlayground.tsx
...
2026-06-12 | Claude Opus 4.8 | Kitsune sprites → true pixel art. Extracted per-frame pixel data from kitsune_chibi_pixel_*.png (dedicated pixel sheets) via a Python+Pillow+scipy pipeline (connected-component frame segmentation → downsample 48px → median-cut 20-colour quantize) baked into kitsunePixels.ts. pixelKitsune.ts renders from that data (no PNG at runtime, no white-bg/crop issues). Command Center + welcome screen use it; welcome character now small+animated (PixelKitsune.tsx) so the activity overview isn't covered. CC strings i18n'd (cc_* keys, en/id/ja). Removed dead kitsuneSheet.ts | kitsunePixels.ts, pixelKitsune.ts, PixelKitsune.tsx, KitsuneCommandCenter.tsx, CodeEditor.tsx, i18n.ts

> **Pixel sprite note:** the kitsune pixel frames in `kitsunePixels.ts` are AUTO-GENERATED (baked char-grid + palette) from `assets/sprites/kitsune_chibi_pixel_*.png`. To change poses/animations, re-run the extraction pipeline (connected-component segmentation → downsample → median-cut quantize) against those source PNGs and overwrite `kitsunePixels.ts`. Don't hand-edit the generated grids. `drawPixelKitsune()` / `<PixelKitsune>` render from this data; nothing loads the PNGs at runtime. The PNG has a WHITE background — `removeWhiteBackground()` flood-fills transparency from the borders at load (preserves whites inside the character). Don't replace it with naive chroma-key: the character has white hair.
